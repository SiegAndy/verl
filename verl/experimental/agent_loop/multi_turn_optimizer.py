...
class TurnStatistics:
    ...
    def add_sample(
        self, generation_time: float, tool_call_time: float, total_time: float
    ):
    ...
    def record_turn(
        ...
        if turn_number not in self.turn_stats:
            self.turn_stats[turn_number] = TurnStatistics(
                turn_number=turn_number, samples_in_turn=0
            )
...
class MultiTurnBatchSynchronizer:
    ...
    async def wait_for_turn_batch(
        self, sample_idx: int, turn_number: int, total_samples_in_turn: int
    ):
        """
...
class EnhancedAgentData:
    ...
    def __init__(self, original_agent_data, sample_idx: int):
        ...
        object.__setattr__(self, "agent_data", original_agent_data)
        object.__setattr__(self, "sample_idx", sample_idx)

        # Turn tracking
        object.__setattr__(self, "current_turn", 0)
        object.__setattr__(self, "turn_start_times", [])
        object.__setattr__(self, "turn_generation_times", [])
        object.__setattr__(self, "turn_tool_call_times", [])

        # Current turn timing
        object.__setattr__(self, "current_generation_start", None)
        object.__setattr__(self, "current_tool_start", None)
    ...
    def get_turn_timings(self) -> List[Dict[str, float]]:
        ...
        for i in range(len(self.turn_start_times)):
            turn_data = {
                "turn": i + 1,
                "generation_time": (
                    self.turn_generation_times[i]
                    if i < len(self.turn_generation_times)
                    else 0.0
                ),
                "tool_call_time": (
                    self.turn_tool_call_times[i]
                    if i < len(self.turn_tool_call_times)
                    else 0.0
                ),
            }
            turn_data["total_time"] = (
                turn_data["generation_time"] + turn_data["tool_call_time"]
            )
            timings.append(turn_data)
        return timings

    def __setattr__(self, name, value):
        """Proxy attribute writes to original agent_data for non-internal attributes."""
        # For internal tracking attributes, set directly on this object
        if name in object.__getattribute__(self, "_internal_attrs"):
            object.__setattr__(self, name, value)
        else:
            # For AgentData attributes, set on the wrapped object
            setattr(object.__getattribute__(self, "agent_data"), name, value)
    ...
    def __delattr__(self, name):
        """Proxy attribute deletion to original agent_data for non-internal attributes."""
        if name in object.__getattribute__(self, "_internal_attrs"):
            object.__delattr__(self, name)
        else:
            delattr(object.__getattribute__(self, "agent_data"), name)
