...
class OptimizedToolAgentLoop(ToolAgentLoop):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        if self.log_turn_statistics and self.stats_collector:
            ...
            if hasattr(output, "extra_fields") and "_turn_timings" in output.extra_fields:
    ...
    async def _handle_generating_state(
        ...
        if not isinstance(agent_data, EnhancedAgentData):
            ...
            enhanced_data = EnhancedAgentData(agent_data, sample_idx)
            # Copy state from original
            for attr in ['messages', 'image_data', 'video_data', 'metrics', 
                        'request_id', 'tools_kwargs', 'prompt_ids', 'response_ids',
                        'response_mask', 'response_logprobs', 'turn_scores', 
                        'tool_rewards', 'user_turns', 'assistant_turns', 
                        'tool_calls', 'extra_fields', 'interaction', 'interaction_kwargs']:
                if hasattr(agent_data, attr):
                    setattr(enhanced_data, attr, getattr(agent_data, attr))
            agent_data = enhanced_data
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        if not isinstance(agent_data, EnhancedAgentData):
            # This shouldn't happen if generation handler wrapped it
            sample_idx = agent_data.extra_fields.get("index", 0)
            enhanced_data = EnhancedAgentData(agent_data, sample_idx)
            for attr in dir(agent_data):
                if not attr.startswith('_') and hasattr(agent_data, attr):
                    try:
                        setattr(enhanced_data, attr, getattr(agent_data, attr))
        ...
        if self.enable_turn_batching and self.batch_synchronizer:
            ...
            total_in_turn = len(agent_data.tool_calls) if hasattr(agent_data, 'tool_calls') else 1
        ...
        if self.log_turn_statistics and self.stats_collector:
            turn_num = getattr(agent_data, '_pending_turn_record', agent_data.assistant_turns)
